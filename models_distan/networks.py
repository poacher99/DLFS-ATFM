...
import math
from .AgeTransformer import ATF
...
class Distan_Generator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, style_dim=50, n_downsampling=2,
        ...
        self.atf = ATF(style_dim=style_dim)
    ...
    def encode(self, input):
        if torch.is_tensor(input):
            ...
            # age_features = self.age_encoder(input)
            return id_features, struct_features, text_features
        else:
            return None, None
    ...
    def forward(self, input, target_age_code, cyc_age_code, source_age_code, disc_pass=False):
        ...
        if disc_pass:
            ...
            # rec_out = self.decode(orig_structure_feat, orig_texture_feat, target_age_features = age_orig_latent)
            rec_out = self.decode(orig_structure_feat, orig_texture_feat, age_orig_latent)
        ...
        if disc_pass:
            ...
            age_cyc_latent, cyc_kl_loss = self.atf(fake_texture_feat, cyc_age_code)
            kl_loss += cyc_kl_loss
            ...
            # cyc_out = self.decode(fake_structure_feat, fake_texture_feat, target_age_features = age_cyc_latent)
            cyc_out = self.decode(fake_structure_feat, fake_texture_feat, age_cyc_latent)
        return rec_out, gen_out, cyc_out, orig_structure_feat, fake_structure_feat, kl_loss.mean()


    def infer(self, input, target_age_features, traverse=False, deploy=False, interp_step=0.5):
        id_features, structure_feat, texture_feat = self.id_encoder(input)
        age_transfer_latent, _ = self.atf(texture_feat, target_age_features)
        # out = self.decode(structure_feat, texture_feat, age_transfer_latent, traverse=traverse, deploy=deploy, interp_step=interp_step)
        out = self.decode(structure_feat,text_features=texture_feat, target_age_features=age_transfer_latent, traverse=traverse, deploy=deploy, interp_step=interp_step)
        return out
